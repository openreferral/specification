...
To this end, HSDS makes several provisions to ensure that the standard can be effectively adapted for use in a given context. The most powerful and impactful of these is the *HSDS Profiles* mechanism, by which people can collaboratively build upon HSDS as a base in ways that further shape the standard for a particular purpose.

## Additional Fields (Extending the schema)

HSDS is a permissive standard which means that data is considered valid as long at it meets
